...
class Weeklymain(models.Model):
    slip_date = models.DateTimeField('slip_date')
    ...
    def recent_slips(self):
        now = timezone.now()
        return now - datetime.timedelta(days=4) <= self.slip_date <= now
...
class Weeklysub(models.Model):
    weekly_main = models.ForeignKey(Weeklymain, on_delete=models.CASCADE, default=None)
